...
Gaussian and Savitzky–Golay smoothing wrap around each waveform's cycle boundary, so the end and beginning are filtered as neighbours.
...
`--sat DRIVE` and `--satbias BIAS` drive the waveform through `tanh(DRIVE * y + BIAS)` and re-centre it. With zero bias, saturation compresses both polarities symmetrically and changes the waveform's shape and harmonic content, even after levels are matched. Bias makes the compression asymmetric and can add even harmonics to an otherwise half-wave-symmetric waveform such as a sine; a saw already contains even harmonics. This asymmetric compression models an analog oscillator's buffer stage acting on a ramp. `--saw ramp --sat 1.257 --satbias 0.625` reproduces a captured hardware saw to a correlation of 0.9935 and a spectral error of 0.62 dB over 64 harmonics, against 1.81 dB for an ideal saw. Both are morphable, but note that a saw driven hard enough becomes a square - at `--sat 8` the waveform correlates 0.96 with one - so sweep the bias, or sweep `--harmonics` and leave the drive where it sounds right.
